...
# Seedance Creator Skill for Claude Code

A Claude Code custom skill that turns Claude into an AI content creator for ByteDance's **Seedance 2.0** (即梦) platform. Generate images and videos directly via dreamina CLI, or get production-ready prompts for the web UI.
...
- A Seedance 2.0 (即梦) account at [jimeng.jianying.com](https://jimeng.jianying.com)
...
dreamina CLI enables direct generation from command line without copying prompts to the web UI.
...
> **Note**: dreamina CLI login requires access to ByteDance servers. If browser doesn't open automatically, use `dreamina login --debug` for debug info.
...
Then type `/seedance` or simply describe your idea. Claude will activate the Seedance skill automatically.
...
Start Claude Code and describe what you want to create:
...
- Mention keywords like: `Seedance`, `即梦`, `视频提示词`, `视频生成`, `AI视频`, `短剧`, `广告视频`, `视频延长`, `生成图片`, `文生图`, `图生图`, `图生视频`, `文生视频`
- Ask to "generate video prompts", "create Seedance prompts", or "generate an image"
...
```
Step 1: Describe your idea → "一段古风武侠打斗" or "生成一张水墨山水画"
...
Step 4: Execute (if dreamina CLI installed) → Auto-call dreamina commands
Step 5: Auto-download → Generated files saved to ./downloads
```

### dreamina CLI Commands (if installed)

With dreamina CLI installed, the skill can execute generation tasks directly:
...
# Text to image
dreamina text2image --prompt="Qi Baishi style ink wash landscape" --ratio=16:9 --resolution_type=2k --poll=60 --download_dir=./downloads

# Text to video
dreamina text2video --prompt="camera push, a cat jumping off sofa" --duration=5 --ratio=16:9 --video_resolution=720P --poll=60 --download_dir=./downloads

# Image to image
dreamina image2image --images ./input.png --prompt="convert to watercolor style" --resolution_type=2k --poll=60 --download_dir=./downloads

# Image to video
dreamina image2video --image ./first_frame.png --prompt="camera slowly push in" --duration=5 --poll=60 --download_dir=./downloads
...
> **Important**: Generated files are automatically saved to `./downloads` directory.
...
### Example 1b: Direct Image Generation (with dreamina CLI)

**Input:**

```
Generate an ink wash landscape painting in Qi Baishi style
```

**Output:**

Skill auto-executes dreamina CLI and saves to `./downloads/shanshui.png`:

```bash
dreamina text2image \
  --prompt="Qi Baishi style ink wash landscape painting, layered ink tones, distant mountains
            with clouds and mist, wide river below with a small boat, fisherman angling,
            vigorous brushstrokes, vivid ink wash artistry, traditional Chinese aesthetic" \
  --ratio=16:9 \
  --resolution_type=2k \
  --poll=60 \
